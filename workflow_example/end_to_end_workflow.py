...
def _(experiment_config, mo, prepared_probe, registration_figure):
    ...
    mo.vstack(
        [registration_summary, registration_figure] if registration_figure is not None else [registration_summary]
    )
    return
...
def _(experiment_config, json):
    ...
    jacobian_paths = tuple(experiment_config["experiment_dir"] / filename for filename in jacobian_filenames)
    return (
        jacobian_paths,
        optical_properties,
